...
export default function SurveyListItem({ survey, onDeleteClick }) {
  return (
    <div className=' flex flex-col py-4 px-6 shadow-md bg-cyan-200 hover:bg-cyan-100 h-[470px]'>
      ...
      <div className=' flex justify-between items-center mt-3'>
        <TButton to={`/survey/${survey.id}`}>
          <PencilIcon className='w-5 h-5   mr-2 text-green-400' />Edit
        </TButton>
        <div className=' flex items-center'>
          ...
          {
            survey.id && (
              <TButton onClick={()=>onDeleteClick(survey.id)} circle link >
